  ...
  if (optimization) {
    ...
    if (
      ...
      if (Array.isArray(treeShaking.modules) && treeShaking.modules.length) {
        const {
          parseUserApis
        } = require('@yump/uni-cli-shared/lib/api')
        try {
          const modules = require('@yump/uni-h5/lib/modules.json')
...
if (process.env.UNI_PLATFORM !== 'h5' && process.env.UNI_PLATFORM !== 'app-fox') {
  try {
    ...
    let info = ''
    if (process.env.UNI_PLATFORM === 'app-plus') {
      const pagesPkg = require('@yump/webpack-uni-pages-loader/package.json')
      if (pagesPkg) {
...
moduleAlias.addAlias('vue-template-compiler', '@yump/vue-cli-plugin-uni/packages/vue-template-compiler')
moduleAlias.addAlias('@megalo/template-compiler', '@yump/vue-cli-plugin-uni/packages/@megalo/template-compiler')
moduleAlias.addAlias('mpvue-template-compiler', '@yump/vue-cli-plugin-uni/packages/mpvue-template-compiler')
