...
const WebpackErrorsPlugin = require('@yump/vue-cli-plugin-uni/packages/webpack-errors-plugin')
const WebpackUniMPPlugin = require('@yump/webpack-uni-mp-loader/lib/plugin/index-new')

const onErrors = require('@yump/vue-cli-plugin-uni/util/on-errors')
const onWarnings = require('@yump/vue-cli-plugin-uni/util/on-warnings')
...
if (
  ...
  provide['__f__'] = [require.resolve('@yump/vue-cli-plugin-uni/lib/format-log.js'), 'default']
}
...
const rules = [{
  ...
  use: [{
    loader: '@yump/webpack-uni-pages-loader'
  }],
  type: 'javascript/auto'
}, {
  test: path.resolve(process.env.UNI_INPUT_DIR, getNVueMainEntry()),
  use: [{
    loader: '@yump/vue-cli-plugin-hbuilderx/packages/webpack-uni-nvue-loader/lib/main'
  }]
}, {
  test: /\.js$/,
  use: [{
  ...
  exclude (modulePath) {
    return excludeModuleReg.test(modulePath) && modulePath.indexOf('@yump') === -1
  }
},
{
  test: /\.nvue(\?[^?]+)?$/,
...
if (process.env.UNI_USING_NVUE_COMPILER) {
  rules.unshift({
    ...
    use: [{
      loader: '@yump/vue-cli-plugin-hbuilderx/packages/webpack-uni-nvue-loader/lib/template'
    }]
  })
}
rules.unshift({
  ...
  use: [{
    loader: '@yump/vue-cli-plugin-hbuilderx/packages/webpack-uni-nvue-loader/lib/template.recycle'
  }]
})
